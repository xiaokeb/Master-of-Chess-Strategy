...
void Network::verify(const std::function<void(std::string_view)>& f,
    ...
    if (evalFile.current != evalfilePath)
    {
        ...
        // Library integrations must report a recoverable error to their host
        // process instead of terminating the Android application.
        throw std::runtime_error("Pikafish NNUE network was not loaded");
    }
