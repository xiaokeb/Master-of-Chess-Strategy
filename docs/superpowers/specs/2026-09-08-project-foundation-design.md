...
- C++ 核心保持平台无关，但统一由指定 NDK LLVM 构建，不调用 MinGW；Android 构建验证编译和链接，设备或 AVD 可用后执行原生测试。
...
3. C++ 平台无关逻辑使用指定 NDK LLVM 构建 Android 测试目标；设备不可用时仅标记编译和链接通过。
4. `engine-api` 使用 JVM 单元测试，避免不必要的模拟器启动。
5. JNI 链路只保留少量 Android 集成测试；Compose UI 测试在出现真实交互流程时添加。
6. 首个切片结束执行一次完整门禁：C++ Android 目标编译、Android 单元测试、Lint 和 Debug APK 构建；设备可用时再加入原生运行测试。
...
6. `engine-native` 能由指定 NDK LLVM 使用 C++17 编译并链接测试目标；设备不可用时准确记录未运行。
7. Android 应用可调用 JNI 健康检查；原生不可用时能够显示错误状态而不崩溃。
8. Gradle/JDK/NDK/CMake 版本明确且可复现。
9. 完整门禁命令的最新输出证明 C++ Android 目标编译、Android 单元测试、Lint 和 Debug 构建通过；若受设备条件阻塞，报告准确的已验证范围和阻塞证据。
