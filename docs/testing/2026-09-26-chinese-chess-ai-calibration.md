...
| 大师 / 困难 | 12 / 0 / 0 / 0 | 100% | 100% | 1147.85 / 1332.83 |

每组 12 局不是 12 个独立开局样本；同一开局交换颜色形成配对。样本少，不能据此推断总体胜率或把它当作发布棋力结论。三组均为修复搜索末层困毙遗漏之前的基线。
...
- 后续补测大师使用同一已内置 NNUE，本机与 AVD 均校验 SHA-256 为 `7d13d73569a9b571ba0eb20cf1596247bc2a42738967e61afef6482b231e900e`，大小 50,706,378 字节。未新增或替换第三方权重，Pikafish/NNUE 的现有发布限制不变；仅传文件路径不等于许可审查。
- 大师每步预算 1200 毫秒，但耗时还包括首次初始化、合法着生成和适配器开销；同一进程复用引擎缓存。测量期间有短时诊断任务与之并行，耗时仅是开发观察值，不是隔离环境性能基准。
...
- `results/2026-09-26-ai-hard-master-v1.jsonl`：12 局、460 次搜索；大师均获胜。首轮程序运行结束后才替换为困毙修复版本，不混用两版引擎结果。
