...
- JVM 单元测试：73 个通过，0 失败、0 错误、0 跳过。
...
1. 按已批准 GPL-3.0 方案固定并接入 Pikafish，完成中国象棋大师难度。
2. 实现正式计时、超时判负、提和、自动演局和调速。
3. 按官方棋例继续扩充杀着、联合捉、暗根、假根、少根、价值交换及追捉集合连续性判定。
4. 在设备或 AVD 可用后运行 Room、Compose、中国象棋与 NativeBridge 集成测试及 AI 性能基准。
